...
export const withHandlingErrorResultCode = function <T>(operation: OperationType<T>, dispatch: any, actions: IActions) {
    return async () => {
...
export const withTryCatch = <T>(operation: OperationType<T>, dispatch: any, actions: IActions) => async () => {
    try {
...
export const withProcessVisualization = function <T>(operation: OperationType<T>, dispatch: any, actions: IActions) {
    return async () => {
...
export let commonAsyncHandler = function <T = any>(operation: OperationType<T>, dispatch: any, actions: IActions) {
...
// First operation commonAsyncHandler
    const withHandlingErrorResultCode = function <T>(operation: OperationType<T>, dispatch: any) {
        return async () => {
            let typedResult: T = await operation()
            let result: any = typedResult;
            if (result && result.resultCode && result.resultCode > ActionResultCodes.Success && result.messages && result.messages.length > 0
...
// Second operation commonAsyncHandler
    const withTryCatch = <T>(operation: OperationType<T>, dispatch: any) => async () => {
        try {
            return await operation()
        } catch (error) {
            let err = error as ErrorType
...
// Third operation commonAsyncHandler
     const withProcessVisualization = function <T>(operation: OperationType<T>, dispatch: any) {
        return async () => {
            actions.setRequestStatus && dispatch(actions.setRequestStatus(RequestStatuses.InProgress))
            const result = await operation()
            actions.setRequestStatus && dispatch(actions.setRequestStatus(RequestStatuses.Finished))
    ...
    let commonAsyncHandler = function <T = any>(operation: OperationType<T>, dispatch: any) {
