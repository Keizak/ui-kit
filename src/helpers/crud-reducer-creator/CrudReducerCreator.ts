...
export const crudReducerCreator = <
    ...
    const getItemById: any =
        (
            userId: number
        ): (dispatch: any) => Promise<TEntity | null> =>
            async (dispatch: any) => {
    ...
    const getItems: any =
        (
        ...
        ): (dispatch: any, getState: () => AppStateType) => Promise<IItemsResult<TEntity> | null> =>
            async (dispatch: any, getState: () => AppStateType) => {
                return commonAsyncHandler(async () => {
                    ...
                    // @ts-ignore
                    let pageSize = newPageSize ? newPageSize : getState().labsReducers[reducerName].pageSize
                    // @ts-ignore
