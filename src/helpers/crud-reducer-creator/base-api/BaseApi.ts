...
> {
  ...
  async path(
    id: number | string,
    model: TPatch
  ): Promise<IActionResult<TEntity>> {
    let response = await this.axios.patch<IActionResult<TEntity>>(
      `${this.baseUrl}/${id}`,
      model
    );
  ...
  async create(): Promise<ICreateActionResult<TEntity>> {
    ...
    const data = {};
    let response = await this.axios.post<ICreateActionResult<TEntity>>(
      `${this.baseUrl}`,
      data
    );
  ...
  async update(
    ...
    let response = await this.axios.put<IActionResult<TEntity>>(
      `${this.baseUrl}/${id}`,
      model
    );
  ...
  async delete(id: number | string): Promise<IActionResult<null>> {
    let response = await this.axios.delete<IActionResult<null>>(
      `${this.baseUrl}/${id}`
    );
  ...
  protected anyGet<T>(endpointUrl: string, params: any = {}, settings?: any) {
    if (endpointUrl[0] === '/')
      endpointUrl = endpointUrl.slice(1, endpointUrl.length);
    ...
    return this.axios
      .get<T>(this.baseUrl + '/' + endpointUrl, defaultSettings)
      .then((r: any) => r.data);
  }
