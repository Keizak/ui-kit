...
import {FormElementType, GridItemElementType, IBaseEntity, RowDataType} from "../../types/types";
...
export const withCRUDGrid = <TUpdate,
    ...
    settings: {
        ...
        getExtraGetAllParams?: (params: { queryParams: { [key: string]: string | undefined } }) => {
    ...
    const CRUDGrid = (props: { renderContentAboveTable?: () => ReactElement }) => {
        //-----------------------GET VALUES FROM HOOKS-------------------------
        let dispatch = appHooks.useAppDispatch()
        // @ts-ignore
        let searchTerms = appHooks.useHostLabsSelector((state: AppStateType) => state[entityName].searchTerms as unknown as CrudStateType<TUpdate, TEntity>)
        // @ts-ignore
        let crudState = appHooks.useHostLabsSelector((state: AppStateType) => state[entityName] as unknown as CrudStateType<TUpdate, TEntity>)
        ...
        const rows: Array<RowDataType<TEntity, React.ReactNode, React.ReactNode>> = items.map(
            (item: TEntity) =>
                createRows<TEntity, React.ReactNode, React.ReactNode>(
                    item,
                    <Tooltip title="Edit">
                        <Link to={navigateUrl(rootRoutingKey as RootRoutingKeys, 'edit', item?.id)}>
                    ...
                    <Tooltip title="Details">
                        <Link
                            to={navigateUrl(rootRoutingKey as RootRoutingKeys, 'details', item?.id)}>
                            <Fab size="small" color="primary"
        ...
        let context = {
            ...
            cancelRedirectUrl: navigateUrl(rootRoutingKey as RootRoutingKeys, 'index'),
        ...
        useEffect(() => {
            settings?.componentDidMountLogic && settings?.componentDidMountLogic(dispatch)
        }, [])
            ...
            <Container fixed>
                <div className={classes.root}>
                    <div style={{display: 'flex', width: '100%'}}>
                        <div style={{width: '15%'}}>
                            <Grid>{props.renderContentAboveTable && props.renderContentAboveTable()}</Grid>
                            {settings?.showCreate ? (
                                <Grid>
                                    {entityName && (
                                        <Button
                                                        ...
                                                        <Component
                                                            ...
                                                            handleOnChange={(values: any) =>
                                                                actions.setSearchTerms(
                                                                    d.editModePropName ? d.editModePropName : d.name,
                                                                    values,
                                                    ...
                                                    {gridStructureData
                                                        ...
                                                        .map((d, r) => {
                                                            ...
                                                            if (d.name) {
                                                                let rawValue: any
                                                                if (d.pathToProp) {
                                                                    rawValue = (row.item as any)[d.pathToProp][d.name]
                                                                } else {
